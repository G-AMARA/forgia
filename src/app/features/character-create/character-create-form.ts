...
// Form di creazione di un nuovo eroe nella Fucina (app-character-create): a differenza
// del vecchio flusso, non naviga più via router al termine, emette solo l'id del PG
// creato e lascia alla Fucina decidere cosa mostrare (vedi CharacterCreate.onHeroCreated).
@Component({
  selector: 'app-character-create-form',
  standalone: true,
  imports: [FormsModule, NgTemplateOutlet, DiceRoller],
  templateUrl: './character-create-form.html',
})
export class CharacterCreateForm {
  ...
  async submit() {
    // Il limite vero è imposto lato RLS (get_character_creation_limit): questo controllo
    // evita solo un errore di permessi poco chiaro se la quota si è esaurita nel frattempo.
    if (!this.characterStore.canCreateCharacter()) {
      this.modal.error(this.localeService.t('character_quota_reached_hint'));
      return;
    }
